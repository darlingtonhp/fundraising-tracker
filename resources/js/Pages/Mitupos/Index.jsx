...
export default function Index({ auth, mitupos }) {
    ...
    const { data: createData, setData: setCreateData, post: createPost, errors: createErrors, reset: resetCreate } = useForm({
        name: "",
        description: "",
    });

    const { data: editData, setData: setEditData, put: editPut, errors: editErrors, reset: resetEdit } = useForm({
        name: "",
        description: "",
    });
    ...
    const filteredAndSortedMitupos = useMemo(() => {
        let filtered = mitupos.filter(mitupo => {
            // Search filter
            const matchesSearch = mitupo.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                                (mitupo.description && mitupo.description.toLowerCase().includes(searchTerm.toLowerCase()));
                    ...
                    <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                        <div className="p-6">
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                {/* Description Filter */}
                                <div>
                                    <InputLabel htmlFor="descriptionFilter" value="Description Filter" />
                                    <select
                            ...
                            {(searchTerm || descriptionFilter !== "all") && (
                                <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                                    <div className="flex flex-wrap items-center gap-2 text-sm">
                                        ...
                                        {descriptionFilter !== "all" && (
                                            <span className="bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-200 px-2 py-1 rounded-full text-xs">
                                                {descriptionFilter === "with" ? "With Description" : "Without Description"}
                                            </span>
                                        )}
                                    </div>
                                </div>
                            ...
                            {filteredAndSortedMitupos.length === 0 ? (
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-gray-500 dark:text-gray-400 text-lg mb-4">
                                        {mitupos.length === 0 ? "No mitupo found." : "No mitupo match your search criteria."}
                                    </p>
                                    {mitupos.length === 0 ? (
                                        <button
                                            onClick={() => setShowCreateForm(true)}
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                            {filteredAndSortedMitupos.map((mitupo) => (
                                                <tr key={mitupo.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                    {editingId === mitupo.id ? (
                                                        // Edit Mode Row
                                                        <td colSpan="4" className="px-6 py-4">
                                                            <form onSubmit={(e) => handleEditSubmit(e, mitupo)} className="space-y-4">
                                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                                    <div>
                                                                        <InputLabel htmlFor={`edit_name_${mitupo.id}`} value="Mutupo Name *" />
                                                                        <TextInput
                                                                    ...
                                                                    <div>
                                                                        <InputLabel htmlFor={`edit_description_${mitupo.id}`} value="Description" />
                                                                        <TextInput
                                                        ...
                                                        <>
                                                            ...
                                                            <td className="px-6 py-4 whitespace-nowrap">
                                                                <div className="text-sm text-gray-500 dark:text-gray-400">
                                                                    {new Date(mitupo.created_at).toLocaleDateString()}
                                                                </div>
                                                            </td>
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                                <div className="flex space-x-3">
                                                                    ...
                                                                    <Link
                                                                        ...
                                                                        onClick={(e) => {
                                                                            if (!confirm('Are you sure you want to delete this mutupo?')) {
                                                                                e.preventDefault();
                                                                            }
                                                                        }}
                                                                    >
                            ...
                            <div className="flex justify-between items-center">
                                <div>
                                    <p className="text-blue-800 dark:text-blue-300 font-semibold">
                                        Showing {filteredAndSortedMitupos.length} of {mitupos.length} mitupo
